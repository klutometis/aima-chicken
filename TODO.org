* TODO Documentation!
  ...
  #+BEGIN_SRC scheme :tangle doc.scm
    ...
    (define (parse-docexpr document docexpr)
      (let ((doc (docexpr-doc docexpr)))
        (match (docexpr-expression docexpr)
          ((? null-expression?)
           ((parse-directive) doc document))
          (('define (procedure . formals) . body)
           ((parse-procedure) doc procedure formals))
          (('define procedure ('lambda formals . body))
           ((parse-procedure) doc procedure formals))
          (('define procedure ('case-lambda (formals . body) ...))
           ((parse-case-lambda) doc procedure formals))
          (('define parameter ('make-parameter init . converter))
           ((parse-parameter) doc parameter))
          (('define scalar . body)
           ((parse-scalar) doc scalar))
          (('define-syntax keyword . _)
           ((parse-syntax) doc keyword))
          (_ 'unknown))))
    ...
    (define tex-case-lambda-procedure
      "\\texttt{(@PROCEDURE@@FORMALS@)} & $\\to$ & \\texttt{@RETURN@}")
    ...
    (define (tex-parse-directive doc document)
    ...
    (define (tex-parse-procedure doc procedure formals)
      (let ((descriptions (filter string? doc))
            (parameters (filter pair? doc)))
        (let ((return (alist-ref/default parameters
                                         '@return
                                         "\\emph{unspecified}")))
          (lambda ()
            (let ((procedure
                   (substitute-template
                    tex-procedure
                    `((procedure . ,procedure)
                      (arguments . ,(string-join (map ->string formals)))
                      (return . ,return))))
                  (description
                   (substitute-template
                  ...
                  (parameters
                   (map
                    ...
                    parameters)))
              (let ((parameters (substitute-template
                                 tex-parameters
                                 'parameters
                ...
                (write-template
                 tex-description
                 'items
                 (string-join (list procedure description parameters)))))))))
    
    (define (tex-parse-scalar doc name)
      void)
    
    (define (tex-parse-parameter doc name)
      void)
    
    (define (tex-parse-case-lambda doc name formals+)
      void)
    
    (define (tex-parse-syntax doc keyword)
      void)
    ...
    (define (tex-parse-docexprs docexprs)
      (let ((document (make-document (make-hash-table) (make-stack))))
        (do ((parsed-docexprs (make-stack))
          ...
          (stack-push! parsed-docexprs (tex-parse-docexpr document docexpr)))))
    
    (let ((parsed-docexprs (tex-parse-docexprs (docexprs))))
      (debug (stack->list parsed-docexprs))
