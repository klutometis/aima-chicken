* TODO Documentation!
  ...
  #+BEGIN_SRC scheme :tangle doc.scm
    ...
    (define-record case-lambda
      name
      formals+)
    
    (define-record-printer case-lambda
      (lambda (record out)
        (write (record->vector record) out)))
  ...
  #+BEGIN_SRC tex :tangle doc.tex
    ...
    \begin{description}
      \item[Procedure] \texttt{(foo \emph{bar}) $\to$ baz}
      \item[Description] An optional and more thorough explanation
        involving fooing bars; what this really entails; pre- or
        post-conditions; \&c.
    
        An optional and more thorough explanation
        involving fooing bars; what this really entails; pre- or
        post-conditions; \&c. \ref{foo}
    \end{description}
    \begin{source}
      %% \inputminted[label=foo,frame=leftline]{scheme}{doc.scm}
      %% \captionof{source}{\texttt{foo}\label{foo}}
      \begin{minted}[frame=leftline]{scheme}
    (with-input-from-file "doc.scm"
      (lambda ()
        (do ((expression (read) (read)))
            ((eof-object? expression))
      ...
      \label{foo}
    \end{source}
    \section[\texttt{test-case-lambda}---Just testing case-lambda]{\texttt{test-case-lambda}}
    \begin{description}
    \item[Procedure] 
      \[\left.\begin{tabular}{l}
          \texttt{(test-case-lambda)} \\
          \texttt{(test-case-lambda \emph{a})}
      \end{tabular}\right\}\to baz\]
    \item[Procedure] 
      \begin{tabular}{lcl}
    ...
    \item[Procedure] 
      \begin{tabular}{l}
          \texttt{(test-case-lambda)} \\
          \texttt{(test-case-lambda \emph{a})} \\
          \texttt{(test-case-lambda \emph{a b})} \\
          \texttt{(test-case-lambda \emph{a b . c})}
      \end{tabular} $\to$ \texttt{baz}
    \item[Procedure] \texttt{(test-case-lambda)} $\to$ \texttt{baz}
    \item[Procedure] \texttt{(test-case-lambda \emph{a})} $\to$ \texttt{baz}
    \item[Procedure] \texttt{(test-case-lambda \emph{a b})} $\to$ \texttt{baz}
    \item[Procedure] \texttt{(test-case-lambda \emph{a b . c})} $\to$ \texttt{baz}
